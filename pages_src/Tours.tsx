...
const Tours: React.FC = () => {
  ...
  const [tours, setTours] = useState<ExperienceEntry[]>([]);
  ...
  const loadTours = async () => {
    try {
      const { data, error } = await kibanExperiences.tours();
          ...
          {loading ? (
            <div className="flex justify-center items-center py-32">
              <div className="text-center">
                ...
                <p className="text-brand-body/60 text-sm uppercase tracking-widest">
                  {language === 'pt' ? 'A carregar tours...' : 'Loading tours...'}
                </p>
              </div>
            </div>
          ) : tours.length === 0 ? (
            <div className="text-center py-32">
              <p className="text-brand-body/60 text-lg">
                {language === 'pt' ? 'Nenhum tour disponível no momento.' : 'No tours available at the moment.'}
              </p>
            </div>
          ) : (
            <motion.div
                  ...
                  >
                    <div className="aspect-[4/5] relative overflow-hidden rounded-t-2xl">
                      <img
                        ...
                        alt={language === 'pt' ? tour.title_pt : tour.title_en}
                      ...
                      {tour.difficulty_pt && (
                        <div className="absolute top-6 left-6">
                          <span className="bg-[#0d4357] text-white text-[10px] font-bold px-4 py-2 rounded-full uppercase tracking-widest shadow-lg">
                            {language === 'pt' ? tour.difficulty_pt : tour.difficulty_en}
                          </span>
                        </div>
                      )}
                    </div>
                    <div className="p-8">
                      ...
                      <h3 className="text-xl md:text-2xl font-bold text-brand-navy">
                        {language === 'pt' ? tour.title_pt : tour.title_en}
                      </h3>
